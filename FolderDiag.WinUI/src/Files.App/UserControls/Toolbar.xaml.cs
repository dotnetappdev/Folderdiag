...
namespace Files.App.UserControls
{
	public sealed partial class Toolbar : UserControl
	{
		...
		// FolderDiag: Theme switching (System / Light / Dark)
		private void ThemeMenuItem_Click(object sender, RoutedEventArgs e)
		{
			if (sender is not MenuFlyoutItem item) return;
			var theme = item.Tag?.ToString() ?? "Default";

			// Persist via AppearanceSettingsService
			var settings = Ioc.Default.GetService<IAppearanceSettingsService>();
			if (settings is not null)
				settings.AppThemeMode = theme;
		...
		// FolderDiag: Folder Analysis pane visibility toggle
		// Raise a message via CommunityToolkit.Mvvm so the host page can show/hide the pane.
		private void FolderAnalysisPaneButton_Click(object sender, RoutedEventArgs e)
		{
			bool isOn = FolderAnalysisPaneButton?.IsChecked == true;
			WeakReferenceMessenger.Default.Send(new FolderDiagAnalysisPaneMessage(isOn));
		}
		// End FolderDiag
	}
}
