...
namespace Files.App.Utils
{
	public partial class ListedItem : ObservableObject, IGroupableItem, IListedItem
	{
		...
		// ── FolderDiag: Folder Size Analysis columns ──────────────────────────────────
		// These are populated asynchronously by FolderSizeService after a scan.

		private ulong _folderSizeBytes;
		/// <summary>Total recursive size of this folder in bytes (0 if not yet scanned).</summary>
		public ulong FolderSizeBytes
		{
			get => _folderSizeBytes;
			set
			{
				if (SetProperty(ref _folderSizeBytes, value))
				{
					OnPropertyChanged(nameof(FolderSizeDisplay));
					OnPropertyChanged(nameof(FolderSizePercent));
					OnPropertyChanged(nameof(FolderSizePercentDisplay));
				}
			}
		}

		private ulong _folderAllocatedBytes;
		/// <summary>Cluster-aligned allocated size of this folder in bytes.</summary>
		public ulong FolderAllocatedBytes
		{
			get => _folderAllocatedBytes;
			set
			{
				if (SetProperty(ref _folderAllocatedBytes, value))
					OnPropertyChanged(nameof(FolderAllocatedDisplay));
			}
		}

		private int _folderFileCount;
		/// <summary>Total recursive file count inside this folder.</summary>
		public int FolderFileCount
		{
			get => _folderFileCount;
			set
			{
				if (SetProperty(ref _folderFileCount, value))
					OnPropertyChanged(nameof(FolderFileCountDisplay));
			}
		}

		private int _folderSubfolderCount;
		/// <summary>Total recursive subfolder count inside this folder.</summary>
		public int FolderSubfolderCount
		{
			get => _folderSubfolderCount;
			set
			{
				if (SetProperty(ref _folderSubfolderCount, value))
					OnPropertyChanged(nameof(FolderSubfolderCountDisplay));
			}
		}

		private double _folderSizePercent;
		/// <summary>Percentage of this folder's size relative to its parent (0–100).</summary>
		public double FolderSizePercent
		{
			get => _folderSizePercent;
			set
			{
				if (SetProperty(ref _folderSizePercent, value))
					OnPropertyChanged(nameof(FolderSizePercentDisplay));
			}
		}

		private bool _isFolderSizeLoading;
		/// <summary>True while FolderSizeService is computing the size for this item.</summary>
		public bool IsFolderSizeLoading
		{
			get => _isFolderSizeLoading;
			set => SetProperty(ref _isFolderSizeLoading, value);
		}

		// Formatted display helpers
		public string FolderSizeDisplay        => FolderSizeBytes == 0 ? "—" : FormatBytes((long)FolderSizeBytes);
		public string FolderAllocatedDisplay   => FolderAllocatedBytes == 0 ? "—" : FormatBytes((long)FolderAllocatedBytes);
		public string FolderFileCountDisplay   => FolderFileCount == 0 ? "—" : FolderFileCount.ToString("N0");
		public string FolderSubfolderCountDisplay => FolderSubfolderCount == 0 ? "—" : FolderSubfolderCount.ToString("N0");
		public string FolderSizePercentDisplay => FolderSizePercent == 0 ? "—" : $"{FolderSizePercent:F1}%";
